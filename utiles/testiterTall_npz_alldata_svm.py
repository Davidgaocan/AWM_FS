...
from utiles.testiterTall_all import find_nonzero_indices
...
def testalldata(datapath,r,daname,lam):
    ...
    output_dir = './out/outtop_npz_svm'
    # data_name = os.path.splitext(datapath)[0]
    data_name = os.path.splitext(os.path.basename(datapath))[0]
    output_file_path = os.path.join(output_dir, f'tinue_our_npz_{daname}_{lam}', f'{data_name}.txt')
    ...
    best_len = 1
    best_subset =np.zeros(X.shape[1])
    ...
    with tqdm(total=total_combinations, desc=f"Processing {data_name}") as pbar:
        ...
        for equalvalue in [1e-3,1e-4,1e-5]:
            for lamada in [lam]:
                for K in [1, 3, 5, 7, 9]:
                    ...
                    wpbmfs = weightmaxwiththreeway_t_1(pd.DataFrame(X), pd.DataFrame(Y), t, lamada, K, equalvalue)
                    for i in range(len(X)):
                        if len(wpbmfs.nearhit[i]) == 0:
                            wpbmfs.nearhit[i]=np.array([i]*K)
                    # 获取特征权重和特征排名
                    try:
        ...
        output_optim_file_path = os.path.join(output_dir, f'optim_our_npz_{daname}_{lam}', f'{data_name}.txt')
