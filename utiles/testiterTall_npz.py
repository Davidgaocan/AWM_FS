...
def testalldata(datapath,r,daname,lam):
    ...
    output_dir = './out/outtop_npz_knn'
    # data_name = os.path.splitext(datapath)[0]
    data_name = os.path.splitext(os.path.basename(datapath))[0]
    output_file_path = os.path.join(output_dir, f'tinue_wpbmfs_d_t_{daname}_{lam}', f'{data_name}.txt')
    ...
    best_len = 1
    t = len(X_train) * r if len(X_train) * r<10000 else len(X_train)
    print(len(X_train) )
    # 获取参数组合总数
