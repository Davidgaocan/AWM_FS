...
def testalldata(datapath,r,lam,dname):
    import os

    output_dir = '../out'
    # data_name = os.path.splitext(datapath)[0]
    data_name = os.path.splitext(os.path.basename(datapath))[0]
    output_file_path = os.path.join(output_dir, f'tinue_wpbmfs_d_t_all_m_{dname}_{lam}', f'{data_name}.txt')
    ...
    with tqdm(total=total_combinations, desc=f"Processing {data_name}") as pbar:
            ...
            for m in [16,32]:
                for lamada in [lam]:
                    for K in [1, 3, 5, 7, 9]:
                        pbar.update(1)
                        # 创建 WPBMFS 实例
                        wpbmfs = weightmaxwiththreeway_t_m(X_train, y_train, t, lamada, K, equalvalue,m)
                        # 获取特征权重和特征排名
                        try:
        ...
        output_optim_file_path = os.path.join(output_dir, f'optim_weight_d_t_all_m_{dname}_{lam}', f'{data_name}.txt')
