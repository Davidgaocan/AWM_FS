...
适用于多分类问题
'''
class Simba_sample_weight_kerneldensity_weight:
    ...
    def reliefF(self,outputfile='../out/exmp2/xor_w.csv',sample_way_isSampleWeight=True):
        ...
        for i in sample_index:  # 采样次数
            ...
            for f in self.X.columns:
                ...
                limba= self.get_limba1(self.margin_init[i])

                self.ei[i][f] = round(self.e[i] * (self.W[f_index]),2)
    ...
    def get_limba(self,margin):
        limba=0
        if margin<=0:
            limba=0
        elif (margin<=np.exp(-1))&(margin>0):
            limba=np.log(1/margin)+(np.exp(1)-1)
        else:
            limba=(1/margin)
        return limba

    def get_limba1(self,margin):
        return 0 if margin<=0 else 1/margin

    def get_limba2(self,margin):
