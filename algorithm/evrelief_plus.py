...
class evrelief_plus:
    def __init__(self, X,Y, sample_rate, t, k):
        ...
        self.classModel=EVM.get_model(self.get_nearmisdis_with_w(),self.Y)
    ...
    def getOnePointPairDistance(self,i):
        ...
        weighted_diff = abs((X - X.iloc[i])).drop(i)
    ...
    def getAllPointPairDistance(self):
        ...
        for i in range(len(self.X)):
            arr=self.getOnePointPairDistance(i)
            self.pairDistance.append(arr)
        return self.pairDistance
    ...
    def getDiswithWeight(self,i,h):
        ...
        nm=abs(self.X.iloc[i] - self.X.iloc[h])*self.W
        nm = sum(nm)
        return nm
    ...
    def getProwithNNmodel(self,i):
        margin=self.getonePointDistanceWithW(i)
        return EVM.get_pro_with_nnmodel(self.nnmodel,i,margin)
    ...
    def get_metix_Z(self,w):
        ...
        z=z/z_norm if z_norm!=0 else [0]*(self.n)
    ...
    def get_nearmisdis_with_w(self,t=1):
        ...
        for i in range(len(self.X)):
            ...
            sum_nm = self.getDiswithWeight(i, NearMiss[0])
    ...
    def reliefF(self,T,tolerance,outputfile='../../out/exmp2/xor_w.csv'):
        ...
        for i in range(T):
            ...
            if(self.compare_convergence(w_t,self.W.copy(),tolerance)):
                print('算法收敛')
...
if __name__ == '__main__':
    a=0
    # x,y=utiles.get_scaler_data('../data/valid_data/xor_dataset.data')
    # a=evrelief_plus(x,y,1,1,5)
    # a.reliefF(1,1e-06)
