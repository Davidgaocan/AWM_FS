...
from algorithm.Code_of_EVM import open_set_evm
...
'''
class Simba_finall:
    def __init__(self, X,Y, sample_rate, t, k,sample):
    ...
    # 过滤式特征选择
    def reliefF(self,outputfile='../out/xor/xor_w.csv'):
        sample = self.X
        # print sample
        ...
        for i in sample_index:  # 采样次数
            ...
            for f in self.X.columns:
                ...
                w_range = round(self.e[i] * (self.W[f_index])*self.sample[i], 2)
...
if __name__ == '__main__':
    ...
    Xtrain, ytrain,sample = open_set_evm('../data/xor_dataset.csv')
    ...
    al=Simba_finall(Xtrain,ytrain,1,1,0,sample)
    outputfile='../out/xor/simba_evm_sample.csv'
    al.reliefF(outputfile)
