...
class evrelief:
    ...
    def get_one_z(self, index,w):
        ...
        x_nm=abs(x.iloc[NearMiss]-x.iloc[index])
        x_nh=abs(x.iloc[NearHit]-x.iloc[index])
        nm=np.array(np.mean(x_nm))
        nh=np.array(np.mean(x_nh))
        n_margin=(nm-nh)*self.weight[index]
        return n_margin
    ...
    def reliefF(self,T,tolerance,outputfile='../../out/exmp2/xor_w.csv'):
        # model=EVM.get_model(self.get_nearmisdis_with_w(),self.Y)
        ...
        for i in range(T):
            ...
            # self.weight = EVM.get_pro_with_model(model, self.get_nearmisdis_with_w())
