...
适用于多分类问题
'''
class Simba_sample_weight_evm_plus_plus:
    ...
    def reliefF(self, outputfile='../../out/exmp2/xor_w.csv', sample_way_isSampleWeight=True):
        ...
        while(True):
            ...
            limba=[1 for f in self.X.columns]
            ...
            for i in sample_index:  # 采样次数
                ...
                for f in self.X.columns:
                    ...
                    # print(self.margin[i])
                    # limba = 0
                    # limba[f_index] *= (round((np.exp(self.margin[i] * (-1)* (self.weight[i]))) ,2))
                    # if self.margin[i] <= 0:
                    #     # limba = 0
                    #     limba += (np.exp(self.margin[i] * (-1)))*(self.weight[i])
                    # else:
                    #     limba += (np.exp(self.margin[i] * (-1)))*(self.weight[i])

                    e[f_index]+=(round(self.e[i] *  self.weight[i], 2))

                    # w_range[f_index] += round(self.e[i] * (self.W[f_index]) * limba * self.weight[i], 2)

            # print(self.W)

            # w_range=[limba[f]*e[f] for f in range(len(self.X.columns))]
            w_range=[e[f]*self.W[f] for f in range(len(self.X.columns))]
    ...
    def get_final(self,cooling_rate):
        ...
        while(True):
            ...
            for i in range(non_minus_ones_count):
                ...
                if(w_new[max_index] == 1):
                    print('*************当前遍历的特征已经入选：', max_index)
                    continue
                ...
                new_margin = self.get_margin_with_subset(w_new)
