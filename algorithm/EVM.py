...
def set_cover_greedy(universe,subsets,cost=lambda x:1.0):
    ...
    while covered != universe:
        ...
        sample_cover_one.append(len(subsets[max_index] - covered))
    ...
    # print(sample_cover_one)
    # sys.exit()
    # print(cover_indices)
    # sys.exit()

    return cover_indices,sample_cover_one
...
def set_cover(points,weibulls,solver=set_cover_greedy):
    ...
    keep_indices,sample_cover_one = solver(universe,subsets)
    #[363, 118, 12, 158, 237, 59, 327, 242, 164, 102, 215, 88, 440, 83]

    return keep_indices,sample_cover_one
...
def reduce_model(points,weibulls,labels,labels_to_reduce=None):
    ...
    sample_cover_one = np.array([],dtype=int) #[]
    for ulabel in ulabels:
        ...
        if ulabel in labels_to_reduce: 
            ...
            keep_ind,sample_cover_one_ind = set_cover(points[ind],[weibulls[i] for i in ind[0]])

            keep = np.concatenate((keep,ind[0][keep_ind]))  #找出EVs
            sample_cover_one = np.concatenate((sample_cover_one,sample_cover_one_ind))  #找出EVs
            # print(keep)
            # sys.exit()
        else:
            keep = np.concatenate((keep,ind[0]))
            sample_cover_one=np.concatenate((sample_cover_one,[1]*len(ind[0])))

    result_dict = {key: value for key, value in zip(keep, sample_cover_one)}
    sorted_result_dict = {k: v for k, v in sorted(result_dict.items(), key=lambda item: item[1], reverse=True)}

    # points = points[list(sorted_result_dict.keys())]
    points = points[list(sorted_result_dict.keys())]

    # weibulls = [weibulls[i] for i in keep]
    labels = labels[list(sorted_result_dict.keys())]

    return points,list(sorted_result_dict.values()),labels
...
def weibull_fit_parallel(args):
    ...
    # print(dists)
    t=0.1
    if tailsize>=len(dists):
        tailsize=int(len(dists)*t)

    sort_dists=sorted(dists)
    # print(sort_dists)
    if(sort_dists[0]!=sort_dists[-1]):
        while(sort_dists[0]==sort_dists[tailsize]):
            t += 0.1
            if (t > 1):
                break
            tailsize = int(len(dists) * t)
    ...
    # nearest = np.sort(dists)[:tailsize]
    nearest =np.partition(dists, tailsize)
...
def get_prob(margin,y):
    ...
    # row_range = list(range(len(y))) #(0-10773)

    margin=np.array(margin)


    margin_same_class = [margin[np.where(y == label)[0]] for label in uni_y]


    args = list(zip(margin_same_class))  #(第一行距离，第一行为0，y标签)
    weibulls=[]
    for dists in margin_same_class:
        weibulls .append(weibull_fit_parallel(dists))

    # print(weibulls)
    # print(np.where(uni_y == y.iloc[0])[0][0])

    weibulls_all=[weibulls[np.where(uni_y==y.iloc[i])[0][0] ]for i in range(len(y))]
    # print((weibulls_all))
    # print(weibulls[1])

    # print(min(margin))
    # print(np.where(margin==0.0050000000000000044))
    # mr = libmr.load_from_string(weibull_params)
    # probs = mr.w_score_vector(dists)

    probs = []
    for i in range(len(y)):
        mr=(libmr.load_from_string(weibulls_all[i]))
        # print(mr.)

        prob=mr.w_score(margin[i])


        probs.append(prob)

    # weibulls_all=[weibulls for i in uni_y]
    # print(probs)
    # print(probs[686])
    # print(max(probs))
    # sys.exit()
    # p = Pool(cpu_count())
    # probs = np.array(p.map(weibull_eval_parallel, list(zip(margin, weibulls_all))))  # xi到其他点的距离（同一类别）在xi韦布分布上的概率   将距离转化为概率
    #
    # p.close()
    # p.join()
    probs_final=[(pro if pro!=-1 else 1) for pro in probs]
    return probs_final

def get_model(margin,y):
...
def get_pro_with_nnmodel(model,i,margin):
    margin = np.array(margin)
    probs = []

    for j in margin:
        mr = (libmr.load_from_string(model[i]))
...
def get_pro_with_model(model,margin):
    margin = np.array(margin)
    probs = []
...
def get_prob_1(margin,y):
    ...
    margin=np.array(list(margin.values()))

    margin_same_class = [ (margin[np.where((y == label))[0]]) [(margin[np.where((y == label))[0]])>0] for label in uni_y]
...
def open_set_evm(train_fname):
    ...
    with timer("...reducing model"):
        Xtrain,sample,ytrain = reduce_model(Xtrain,weibulls,ytrain)  #模型减少操作
    ...
    return pd.DataFrame(Xtrain),pd.DataFrame(ytrain),sample

# if __name__ == '__main__':
#
#     # Xtrain,ytrain = open_set_evm('../data/xor_dataset.data')
#     #
#     # print(Xtrain)
#     # print(ytrain)
